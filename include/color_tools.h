...
uint16_t getClockDigitColor(int hour, int minute)
{
  ...
  float temp;
  if (timeOfDay < SHOW_HOUR_START || timeOfDay >= SHOW_HOUR_END)  // 10 PM to 7 AM
  {
    // Night time
    temp = NIGHT_TEMP;
  }
  ...
  {
    // Morning: temperature increases (7 AM to 12 PM)
    temp = MIN_TEMP + (MAX_TEMP - MIN_TEMP) * ((timeOfDay - 7) / 5.0f);
  }
  else if (timeOfDay < 17)
  {
    // Afternoon: temperature stays at max (12 PM to 5 PM)
    temp = MAX_TEMP;
  }
  else
  {
    // Evening: temperature decreases to night temp (5 PM to 10 PM)
    temp = MAX_TEMP - (MAX_TEMP - NIGHT_TEMP) * ((timeOfDay - 17) / 5.0f);
  }
  ...
  // Dim the color at night (10 PM to 7 AM)
  if (timeOfDay < SHOW_HOUR_START || timeOfDay >= SHOW_HOUR_END)
  {
    float dimFactor = CONFIG_NIGHT_DIM_FACTOR; // 30% brightness at night
    red *= dimFactor;
    green *= dimFactor;
    blue *= dimFactor;
  }
